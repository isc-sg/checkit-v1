...
try:
    ...
    sql_statement = "SELECT * FROM adm WHERE id = 1"
...
try:
    ...
    sql = "UPDATE main_menu_licensing SET transaction_count =  " + str(transaction_count) + ", " + \
          ...
          "license_key = " + "\"" + license_key + "\"" + " WHERE id = 1"
...
if checkit_result is None:
    ...
    sql_statement = "INSERT INTO main_menu_enginestate " \
                    "(state, engine_process_id, transaction_rate, state_timestamp) VALUES (%s,%s,%s,%s)"
    values = (state, engine_process_id, transaction_rate, state_timestamp)
...
def calculate_transaction_rate():
    sql_statement = "SELECT COUNT(*) FROM main_menu_logimage WHERE creation_date > " + "\"" + state_timestamp + "\""
    ...
    time_elapsed = datetime.datetime.now() - datetime.datetime.strptime(state_timestamp, "%Y-%m-%d %H:%M:%S.%f")
