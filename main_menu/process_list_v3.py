...
list_of_cameras = list(range(1, 100))

cpus = cpu_count()
chunks = math.ceil(len(list_of_cameras)/cpus)
chunked_list = np.array_split(list_of_cameras, chunks)
...
class ProcessCameras(object):
    ...
    def process_list(self, camera):
        print("processing", camera, pathos.helpers.mp.current_process())
        init_pools()
...
def main():
    ...
    p = ProcessCameras()
    pool.imap(p.process_list, chunked_list)
    pool.close()
    pool.join()
